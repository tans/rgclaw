...
export function renderUserCenter(input: RenderUserCenterInput) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>用户中心</title></head><body><main><h1>用户中心</h1><div>邮箱: ${input.email}</div><div>钱包地址: ${input.walletAddress || "未填写"}</div><div>有效期: ${input.entitlementText}</div><ul>${subscriptionItems}</ul></main></body></html>`;
}
